...
/*
TextureCache::LoadResult TextureCache::load(std::string path) {
    auto textureFromCache = cache.find(path);
    if (textureFromCache != cache.end()) {
        printf("Loading cached texture %s\n", path.c_str());
        return LoadResult(true, &textureFromCache->second);
    }
    ...
    if (!pixels) {
        ...
        return LoadResult(false, nullptr);
    }
    ...
    texture.image.imageExtent.width = width;
    texture.image.imageExtent.height = height;
    texture.image.imageExtent.depth = 1;
    ...
    return LoadTexture(true, &texture);
}
*/
